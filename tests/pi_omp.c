...
#define NUM_THREADS 8
    ...
    static unsigned int num_steps = (unsigned long int)((1<<30)-1) ;
    double step;
int main (int argc, char** argv) { 
    ...
    if (argc == 1) {
        printf("num_steps %d, ¿Factor de escala (1..4)?\n", num_steps);
        if (scanf("%d", &factor) <= 0)
...
printf("Pi %25.23f, calc con %d pasos en %f segundos\n", pi,num_steps,segundos);
